"""
BIO distant supervision élargi : 1973–1993 (pre-2000 uniquement)
Sources :
  - archelec_annotated.json          (8670 docs, 1973/1978)
  - archelec_annotated_1981.json     (  ~460 docs, 1981)
  - archelec_annotated_new_years_checkpoint.json (700 docs, 1986/1988/1993)
→ data/bio_distantsup_extended/
...
output_dir = BASE_DIR / "data" / "bio_distantsup_extended"
output_dir.mkdir(exist_ok=True)

annotated_dir = BASE_DIR / "data" / "annotated"
input_files = [
    annotated_dir / "archelec_annotated.json",
    annotated_dir / "archelec_annotated_1981.json",
    annotated_dir / "archelec_annotated_new_years_checkpoint.json",
]
...
print("Chargement tokenizer CamemBERT...")
...
print(f"\nSplit : train={len(train_docs)} | val={len(val_docs)} | test={len(test_docs)}")
...
for nom, docs in [("train", train_docs), ("val", val_docs), ("test", test_docs)]:
    ...
    print(f"  OK {nom}.json ({out.stat().st_size/1024/1024:.1f} MB)")
