"""
Fine-tuning direct : camembert-base → gold annotations uniquement (pas de Stage 1).
Train : data/bio/train.json (46 docs, splits/train.json)
Val   : data/bio/val.json   (30 docs, splits/test_before_2000.json)
Test  : data/bio/test.json  (50 docs, splits/test_after_2000.json)
Permet de comparer avec l'approche 2 stages.
"""
import json
import numpy as np
import torch
...
MODEL_NAME = "camembert-base"  # pas de Stage 1
...
def charger_split(chemin):
    ...
    return Dataset.from_list([
        {'input_ids': d['input_ids'], 'ner_tags': d['ner_tags'],
         'id': str(d['id']), 'annee': str(d['annee'])}
        for d in data
    ])
...
print(f'Chargement {MODEL_NAME} (sans Stage 1)...')
...
args = TrainingArguments(
    output_dir                  = str(MODELS_DIR / 'direct_finetune_checkpoints'),
    num_train_epochs            = 20,
...
print('Fine-tuning direct sur 46 docs gold (sans Stage 1)...')
trainer.train()
print('Fine-tuning termine !')

print('\n=== EVALUATION test_before_2000 ===')
results_val = trainer.evaluate(dataset_proc['val'])
for entite in ['PER', 'ORG', 'LOC', 'MISC']:
    print(f'{entite:<8} F1={results_val.get(f"eval_f1_{entite}", 0):.2f}%')
print(f'GLOBAL   F1={results_val.get("eval_f1_global", 0):.2f}%')

print('\n=== EVALUATION test_after_2000 ===')
results_test = trainer.evaluate(dataset_proc['test'])
for entite in ['PER', 'ORG', 'LOC', 'MISC']:
    print(f'{entite:<8} F1={results_test.get(f"eval_f1_{entite}", 0):.2f}%')
print(f'GLOBAL   F1={results_test.get("eval_f1_global", 0):.2f}%')

with open(RESULTS_DIR / 'direct_finetune_before_2000.json', 'w') as f:
    json.dump(results_val, f, indent=2)
with open(RESULTS_DIR / 'direct_finetune_after_2000.json', 'w') as f:
    json.dump(results_test, f, indent=2)

model_dir = MODELS_DIR / 'direct_finetune_model'
